...
export default function RoleSwitcher({ className }: { className?: string }) {
  ...
  const switchRole = (newRole: 'merchant' | 'admin' | 'employee') => {
    ...
    try {
      ...
      if (newRole === 'merchant') {
        // Pick dynamic first registered buyer or general merchant
        let defaultBuyer = 'Wholesale Merchant';
        try {
          const raw = localStorage.getItem('stakeholders_v2') || localStorage.getItem('users_stakeholders');
          if (raw) {
        ...
        localStorage.setItem('userName', defaultBuyer);
