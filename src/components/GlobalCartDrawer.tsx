...
export default function GlobalCartDrawer() {
  ...
  // State for destination shop management
    ...
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed.suppliers) && parsed.suppliers[0]?.name) {
          return parsed.suppliers[0].name.trim();
        }
      }
    } catch (e) {}
    return 'DIRECT MILL';
    ...
    try {
      ...
      for (const [supplier, supplierItems] of Object.entries(supplierGroups)) {
        ...
        const groupedOrder = {
          ...
          buyer: buyerName || 'Authorized Merchant',
          supplier: supplier,
        ...
        // Auto-dispatch Email, WhatsApp & SMS notifications to buyer and supplier for each PO
        const supplierProf = resolveSupplierProfile(supplier);

        // 1. Send Purchase Order Email to Supplier
        const supplierEmailHtml = generateSupplierPOEmailHtml(supplier, groupedOrder, supplierItems.map(si => ({
          id: batchId,
          buyer: buyerName || 'Valued Merchant',
        ...
        fetch('/api/dispatch-po', {
          ...
          body: JSON.stringify({
            ...
            textContent: `Purchase Order ${batchId} for ${supplier} (${grpQty} QTLS). Total: ₹ ${formatINR(grpAmt)}.`
          })
        })
        .then(res => res.json())
        .then(data => console.log(`Cart Drawer PO Email dispatched to ${supplier}:`, data))
        .catch(err => console.warn(`Cart Drawer PO Email dispatch error for ${supplier}:`, err?.message || err));

        // 2. Send Order Confirmation Email to Buyer
        if (userEmail) {
          const buyerEmailHtml = generateBuyerConfirmationEmailHtml(buyerName, groupedOrder, supplierItems.map(si => ({
            product: si.product.name,
            qty: si.qty,
            rate: si.product.price,
          ...
          fetch('/api/dispatch-po', {
            ...
            body: JSON.stringify({
              ...
              textContent: `Hello ${buyerName}, your procurement request ${batchId} for ${grpQty} QTLS from ${supplier} has been registered.`
            })
          })
          .then(res => res.json())
          .then(data => console.log(`Cart Drawer Buyer Email dispatched to ${userEmail}:`, data))
          .catch(err => console.warn(`Cart Drawer Buyer Email dispatch error:`, err?.message || err));
        }
      ...
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-[100] overflow-hidden">
            ...
            {/* Sliding Drawer: On mobile full width, on desktop 440px */}
            <div className="absolute inset-y-0 right-0 max-w-full flex pl-0 sm:pl-10">
              <motion.div
                ...
                className="w-full sm:w-[440px] max-w-full bg-[#fcfbf9] dark:bg-[#07140f] sm:border-l border-emerald-900/15 dark:border-emerald-950/60 shadow-2xl flex flex-col justify-between h-full sm:rounded-l-3xl overflow-hidden font-sans text-emerald-950 dark:text-emerald-50"
              >
                ...
                <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
                  {cartList.length === 0 ? (
                    ...
                    <div className="space-y-2.5">
                      <div className="flex justify-between items-center pb-1 border-b border-dashed border-emerald-500/15">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="text-[9.5px] font-black text-emerald-700 dark:text-emerald-400 hover:underline flex items-center gap-0.5 cursor-pointer"
                          >
                            <span>Bag at a Glance</span>
                      ...
                      {cartList.map((item) => (
                        <motion.div
                          key={item.product.id}
                          layout
                          className="p-2.5 sm:p-3 bg-white dark:bg-[#0b1c15] rounded-xl border border-emerald-500/15 dark:border-emerald-950/50 shadow-2xs space-y-2 relative text-left"
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-2 min-w-0 flex-1">
                              {item.product.image && (
                                <div className="w-9 h-9 rounded-lg overflow-hidden shrink-0 border border-emerald-500/20 bg-emerald-50 dark:bg-emerald-950/40 p-0.5 flex items-center justify-center">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <h4 className="text-xs font-black text-emerald-950 dark:text-emerald-100 mt-0.5 truncate leading-tight">
                                  {item.product.name}
                                </h4>
                              </div>
                            </div>
                            ...
                            <button
                              ...
                              className="text-rose-500/70 hover:text-rose-600 bg-rose-500/10 p-1 rounded-lg cursor-pointer transition-colors shrink-0"
                              title="Remove item"
                            >
                          ...
                          <div className="flex items-center justify-between gap-2 pt-1.5 border-t border-emerald-500/10 dark:border-emerald-950/30">
                            <div>
                              <p className="text-[7.5px] font-black uppercase text-emerald-900/40 dark:text-emerald-300/40 leading-none">Rate</p>
                              <p className="text-xs font-black text-emerald-950 dark:text-emerald-50 mt-0.5">
                                ₹{formatINR(item.product.price)} <span className="text-[8px] font-normal text-emerald-800/60 dark:text-emerald-300/50">/ Qty</span>
                              </p>
                            </div>
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <div className="flex items-center bg-emerald-500/[0.04] dark:bg-emerald-950/30 border border-emerald-500/20 dark:border-emerald-950/60 rounded-lg overflow-hidden shrink-0">
                                <button
                                  ...
                                  className="px-2 py-0.5 hover:bg-emerald-500/20 text-xs font-black text-emerald-900 dark:text-emerald-200 cursor-pointer"
                                >
                                  -
                                </button>
                                <input
                                  ...
                                  className="w-10 text-center bg-transparent border-none text-[11px] font-black font-mono text-emerald-950 dark:text-emerald-50 p-0 focus:ring-0 focus:outline-none"
                                />
                                <button
                                  type="button"
                              ...
                              <div className="text-right">
                                <p className="text-[7.5px] font-black uppercase text-emerald-900/40 dark:text-emerald-300/40 leading-none">Subtotal</p>
                                <p className="text-xs font-black font-mono text-emerald-700 dark:text-emerald-400 mt-0.5">
                                  ₹{formatINR(item.qty * item.product.price)}
                                </p>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between gap-1 pt-0.5">
                            <span className="text-[7.5px] font-bold text-slate-400 dark:text-slate-500 uppercase font-mono">Quick Add:</span>
                            <div className="flex items-center gap-1">
                              {[25, 50, 100].map((delta) => (
                                <button
                                  key={delta}
                                  type="button"
                                  onClick={() => updateQty(item.product.id, item.qty + delta)}
                                  className="text-[8px] font-bold px-1.5 py-0.5 bg-emerald-500/[0.05] hover:bg-emerald-500/15 border border-emerald-500/15 text-emerald-900 dark:text-emerald-300 rounded transition-all cursor-pointer font-mono"
                                >
                                  +{delta}
                                </button>
                              ))}
                    ...
                    >
                      {/* --- DISPATCH DESTINATION SHOP & GODOWN SELECTOR (COMPACT ACCORDION) --- */}
                      <div className="p-2.5 bg-white dark:bg-[#0b1c15] border border-emerald-500/20 dark:border-emerald-950/60 rounded-xl space-y-2 shadow-2xs">
                        {(() => {
                          const activeLoc = deliveryLocations.find(l => l.id === selectedLocationId) || deliveryLocations[0];
                          return (
                            <div>
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-1.5 min-w-0">
                                  <div className={cn(
                                    ...
                                    activeLoc?.type === 'Godown' ? "bg-amber-500/15 text-amber-700" : "bg-emerald-500/15 text-emerald-700 dark:text-emerald-400"
                                  )}>
                                    {activeLoc?.type === 'Godown' ? <Warehouse className="w-3.5 h-3.5" /> : <Store className="w-3.5 h-3.5" />}
                                  </div>
                                  <div className="min-w-0">
                                    <div className="flex items-center gap-1">
                                    ...
                                    <p className="text-[8.5px] text-slate-500 dark:text-slate-400 truncate">
                                      📍 {activeLoc?.address || 'APMC Yard'}
                                    </p>
                                  </div>
                                </div>
                                ...
                                <button
                                  ...
                                  className="px-2 py-0.5 text-[8.5px] font-bold text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 rounded-md transition-all cursor-pointer shrink-0"
                                >
                                  {isChangingLocation ? 'Done' : 'Change'}
                                </button>
                              </div>

                              {/* Expanded Destination Selection List */}
                              <AnimatePresence>
                                {isChangingLocation && (
                                  <motion.div
                                    ...
                                    className="pt-2 mt-2 border-t border-emerald-500/15 space-y-1.5"
                                  >
                                    <div className="flex items-center justify-between pb-1">
                                      ...
                                      <button
                                        ...
                                        className="inline-flex items-center gap-0.5 text-[8px] font-bold text-emerald-700 dark:text-emerald-400 hover:underline cursor-pointer"
                                      >
                                        <Plus className="w-2.5 h-2.5" />
                                        <span>{isAddingLocation ? 'Cancel Add' : '+ Add New'}</span>
                                      </button>
                                    </div>
                                    ...
                                    {isAddingLocation && (
                                      <form
                                        ...
                                        className="p-2 bg-emerald-50/70 dark:bg-emerald-950/40 rounded-lg border border-emerald-500/20 space-y-1.5 mb-1.5"
                                      >
                                        <div className="flex items-center justify-between">
                                          <span className="text-[8px] font-black uppercase text-emerald-900 dark:text-emerald-200">New Destination</span>
                                          <div className="flex gap-1">
                                            <button
                                              type="button"
                                              onClick={() => setNewLocType('Shop')}
                                              className={cn("px-1.5 py-0.5 rounded text-[7.5px] font-bold cursor-pointer", newLocType === 'Shop' ? "bg-[#0e4835] text-white" : "bg-white text-slate-700")}
                                            >
                                              Shop
                                            </button>
                                            <button
                                              ...
                                              className={cn("px-1.5 py-0.5 rounded text-[7.5px] font-bold cursor-pointer", newLocType === 'Godown' ? "bg-amber-700 text-white" : "bg-white text-slate-700")}
                                            >
                                              Godown
                                            </button>
                                          </div>
                                        ...
                                        <input
                                          type="text"
                                          placeholder="Shop / Godown Name"
                                          value={newLocName}
                                          onChange={(e) => setNewLocName(e.target.value)}
                                          className="w-full px-2 py-1 bg-white dark:bg-[#07130e] border border-emerald-500/30 rounded text-[11px] outline-none"
                                          required
                                        />
                                        <input
                                          type="text"
                                          placeholder="Address / APMC Shed"
                                          value={newLocAddress}
                                        ...
                                        <div className="flex justify-end gap-1">
                                          <button
                                            type="submit"
                                            className="px-2.5 py-0.5 bg-[#0e4835] text-white text-[8px] font-bold rounded cursor-pointer"
                                          >
                                            Save & Select
                                          </button>
                                        </div>
                                      ...
                                      {deliveryLocations.map((loc) => {
                                        ...
                                        return (
                                          <div
                                            ...
                                            className={cn(
                                              "p-1.5 rounded-lg border transition-all cursor-pointer flex items-center justify-between text-left",
                                              isSelected
                                                ? "bg-emerald-500/[0.08] dark:bg-emerald-950/50 border-emerald-600 dark:border-emerald-500"
                                                : "bg-slate-50/60 dark:bg-neutral-900/40 border-slate-200/80 hover:bg-slate-100/60"
                                            )}
                                          >
                                            <div className="min-w-0 flex-1 pr-2">
                                              <p className="text-[10px] font-bold text-slate-900 dark:text-white truncate">
                                                {loc.name} <span className="text-[7.5px] font-mono text-slate-400">({loc.type})</span>
                                              </p>
                                              <p className="text-[8.5px] text-slate-500 truncate">{loc.address}</p>
                                            </div>
                        ...
                        <div className="grid grid-cols-12 gap-1.5 pt-0.5">
                          ...
                          <div className="col-span-8">
                            ...
                            <input
                              ...
                              placeholder="e.g. Authorized Merchant"
                        ...
                        {Object.entries(supplierGroups).map(([supplier, supplierItems]) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {/* Direct Mill Dispatch Routing Route */}
                              <div className="p-1.5 rounded-xl bg-emerald-500/[0.04] dark:bg-emerald-950/25 border border-emerald-500/10 flex items-center justify-between gap-1 text-[8.5px]">
                                <div className="flex items-center gap-1.5 min-w-0 pr-1">
                                  <Truck className="w-3 h-3 text-emerald-600 shrink-0" />
                                  <span className="text-slate-400 dark:text-slate-500 shrink-0">Dispatch Routing:</span>
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button
                  onClick={() => {
                    ...
                    const role = localStorage.getItem('userRole') || 'merchant';
                    navigate(role === 'merchant' ? '/my-orders' : '/placed-orders');
                  }}
