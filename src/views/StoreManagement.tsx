...
function ProductCardMedia({ product }: { product: any }) {
  ...
  if (product.image && !imgError) {
    return (
      <div className="w-full h-44 sm:h-48 relative overflow-hidden flex items-center justify-center bg-gradient-to-b from-stone-50 to-stone-100 dark:from-neutral-900/90 dark:to-neutral-950 p-2.5">
        <img
          src={product.image}
          alt={product.name}
...
export default function StoreManagement() {
  ...
  useEffect(() => {
    ...
    // Safety fallback: ensure loading never hangs beyond 1.2s under any network condition
  ...
  if (loading) {
    return <StoreLoadingScreen minDurationMs={650} />;
  }
      ...
      <div className="w-full px-3 py-3 space-y-3.5">
        ...
        {rateListGraphic && rateListGraphic.imageUrl && (
          <div
            ...
            className="w-full bg-amber-50 dark:bg-[#15120a] hover:bg-amber-100/60 dark:hover:bg-[#1c180e] border border-amber-200/80 dark:border-amber-900/40 rounded-2xl p-2.5 flex items-center justify-between gap-3 shadow-2xs transition-all cursor-pointer group active:scale-[0.99]"
          >
