...
import { cn, formatINR, sanitizeSupplierName } from '../lib/utils';
...
export default function PendingLoadings() {
  ...
  const handleDeleteWholePendingLoadings = async (e: React.FormEvent) => {
    ...
    const check = await verifyActionWithServer('delete_whole_pending_loadings', deletePassword.trim());
    if (!check.success) {
      ...
      placedOrdersList.forEach((ord: any) => {
        ...
        if (ord.originalOrders && Array.isArray(ord.originalOrders) && ord.originalOrders.length > 1) {
          ord.originalOrders.forEach((sub: any, sIdx: number) => {
            ...
            const buyer = sub.buyer || sub.partyName || ord.buyer || 'General Merchant';
          ...
          const supplier = sanitizeSupplierName(ord.supplier || ord.seller || (ord.originalOrders?.[0]?.supplier), 'DIRECT MILL');
          const buyer = ord.buyer || ord.partyName || (ord.originalOrders?.[0]?.buyer) || 'General Merchant';
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto pb-32">
      ...
      <AnimatePresence>
        {selectedItem && (
          <div className="fixed inset-0 bg-neutral-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-4">
            <motion.div 
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-surface border border-outline-variant/50 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[88dvh]"
            >
              <div className="p-4 sm:p-6 border-b border-outline-variant/30 bg-surface-container/50 flex justify-between items-center shrink-0">
                <div>
      ...
      <AnimatePresence>
        {isDeleteAllModalOpen && (
          <div className="fixed inset-0 bg-neutral-950/75 backdrop-blur-md z-50 flex items-center justify-center p-3 sm:p-4">
            <motion.div 
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-surface border border-outline-variant/60 w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[88dvh]"
            >
              {/* Modal Header */}
              <div className="p-4 sm:p-6 border-b border-outline-variant/30 bg-surface-container/50 flex items-start gap-4 shrink-0">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-red-500/20 border border-red-500/40 flex items-center justify-center text-red-600 dark:text-red-400 shrink-0">
                  <ShieldAlert className="w-5 h-5 sm:w-6 sm:h-6" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="text-base sm:text-lg font-black text-on-surface tracking-tight">Delete Whole Pending Loadings</h3>
                    <button
                      onClick={() => {
                        setIsDeleteAllModalOpen(false);
                        setDeletePassword('');
              ...
              {/* Scope Summary */}
              <form onSubmit={handleDeleteWholePendingLoadings} className="p-4 sm:p-6 space-y-5 overflow-y-auto flex-1">
