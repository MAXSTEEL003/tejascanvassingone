    ...
    <>
      <header className={cn(
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Grain Journey Scroll Link - Hidden for Admin Side */}
          {!isAdminSide && (
            <button
              onClick={() => {
                if (window.location.pathname === '/about') {
                  const el = document.getElementById('grain-journey');
                  if (el) {
              ...
              className="px-2.5 py-1.5 text-amber-700 dark:text-amber-300 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/20 rounded-xl transition-all cursor-pointer flex items-center gap-1.5 text-xs font-bold shadow-2xs"
              title="Experience Grain Journey on About Page"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-500 animate-pulse" />
              <span className="hidden sm:inline">Grain Journey</span>
            </button>
          )}
