...
import { 
  ...
  MessageSquare, Mail, Plus, Truck, ChevronDown, Sparkles
} from 'lucide-react';
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
                  {cartList.length === 0 ? (
                    ...
                    <div className="space-y-2.5">
                      ...
                      {cartList.map((item) => (
                        ...
                        >
                          ...
                          <div className="pr-7 text-left flex items-center gap-2.5">
                            {item.product.image && (
                              <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 border border-emerald-500/20 bg-emerald-50 dark:bg-emerald-950/40 p-0.5 flex items-center justify-center">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <h4 className="text-xs font-black text-emerald-950 dark:text-emerald-100 mt-0.5 truncate">
                                {item.product.name}
                              </h4>
                    ...
                    >
                      {/* --- DISPATCH DESTINATION SHOP & GODOWN SELECTOR --- */}
                      <div className="p-3 bg-white dark:bg-[#0b1c15] border border-emerald-500/20 dark:border-emerald-950/60 rounded-2xl space-y-2 shadow-xs">
                        <div className="flex items-center justify-between border-b border-emerald-500/10 pb-1.5">
                          <div className="flex items-center gap-1.5">
                            <div className="w-6 h-6 rounded-lg bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 flex items-center justify-center shrink-0">
                              <Store className="w-3.5 h-3.5 text-emerald-600" />
                            </div>
                            <div>
                              <span className="text-[9.5px] font-black uppercase tracking-wider text-emerald-950 dark:text-emerald-100 block leading-none">
                                Dispatch Destination Shop
                              </span>
                              <span className="text-[8px] text-emerald-800/60 dark:text-emerald-400/60 font-medium">
                                Direct ex-mill lorry routing destination
                              </span>
                            </div>
                          </div>
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-800 dark:text-emerald-300 border border-emerald-500/20 text-[9px] font-extrabold cursor-pointer transition-all active:scale-95"
                          >
                            <Plus className="w-2.5 h-2.5" />
                            <span>{isAddingLocation ? 'Cancel' : 'Add Shop'}</span>
                          </button>
                        </div>

                        {/* Inline Add Destination Shop / Godown Form */}
                        <AnimatePresence>
                          {isAddingLocation && (
                            <motion.form
                              ...
                              className="overflow-hidden bg-emerald-50/70 dark:bg-emerald-950/40 p-2.5 rounded-xl border border-emerald-500/25 space-y-2 text-left"
                            >
                              <div className="flex items-center justify-between">
                                <span className="text-[8.5px] font-black uppercase tracking-wider text-emerald-900 dark:text-emerald-200">
                                  Register New Destination
                                </span>
                                <div className="flex items-center gap-1 bg-white/80 dark:bg-black/30 p-0.5 rounded-lg border border-emerald-500/20">
                                  <button
                                    type="button"
                                    onClick={() => setNewLocType('Shop')}
                                    className={cn(
                                      "px-2 py-0.5 rounded-md text-[8px] font-extrabold transition-all cursor-pointer",
                                      newLocType === 'Shop' ? "bg-[#0e4835] text-white" : "text-slate-600 dark:text-slate-400"
                                    )}
                                  >
                                    🏬 Retail Shop
                                  </button>
                                  <button
                                    ...
                                    className={cn(
                                      "px-2 py-0.5 rounded-md text-[8px] font-extrabold transition-all cursor-pointer",
                                      newLocType === 'Godown' ? "bg-amber-700 text-white" : "text-slate-600 dark:text-slate-400"
                                    )}
                                  >
                                    🏭 Godown
                                  </button>
                              ...
                              <div className="space-y-1.5">
                                <input
                                  type="text"
                                  placeholder="Destination Shop Name (e.g. City Central Depot - Branch 2)"
                                  value={newLocName}
                                  onChange={(e) => setNewLocName(e.target.value)}
                                  className="w-full px-2.5 py-1.5 bg-white dark:bg-[#07130e] border border-emerald-500/30 rounded-lg text-xs font-bold text-slate-900 dark:text-white outline-none focus:ring-1 focus:ring-emerald-500"
                                  required
                                />
                                <input
                                  type="text"
                                  placeholder="Delivery Address / APMC Yard Shed Number"
                                  value={newLocAddress}
                                  onChange={(e) => setNewLocAddress(e.target.value)}
                                  className="w-full px-2.5 py-1.5 bg-white dark:bg-[#07130e] border border-emerald-500/30 rounded-lg text-xs font-medium text-slate-900 dark:text-white outline-none focus:ring-1 focus:ring-emerald-500"
                                  required
                                />
                                <input
                                  type="text"
                                  placeholder="Contact Phone / Gate In-charge (optional)"
                                  value={newLocPhone}
                              ...
                              <div className="flex items-center justify-end gap-1.5 pt-0.5">
                                <button
                                  ...
                                  className="px-2.5 py-1 rounded-lg text-[9px] font-bold text-slate-500 hover:text-slate-700 dark:text-slate-400 cursor-pointer"
                                >
                                  Cancel
                                </button>
                                <button
                                  type="submit"
                                  className="px-3 py-1 rounded-lg bg-[#0e4835] hover:bg-[#093526] text-white text-[9px] font-extrabold shadow-2xs cursor-pointer active:scale-95 transition-all"
                                >
                                  Save & Select
                                </button>
                              </div>
                          ...
                          {deliveryLocations.map((loc) => {
                            ...
                            return (
                              <div
                                ...
                                className={cn(
                                  "p-2.5 rounded-xl border transition-all cursor-pointer flex items-start justify-between gap-2 text-left relative",
                                  isSelected
                                    ? "bg-emerald-500/[0.08] dark:bg-emerald-950/50 border-emerald-600 dark:border-emerald-500 ring-1 ring-emerald-500/40 shadow-xs"
                                    : "bg-slate-50/60 dark:bg-neutral-900/40 border-slate-200/80 dark:border-neutral-800/80 hover:bg-slate-100/60 dark:hover:bg-neutral-850/60"
                                )}
                              >
                                <div className="flex items-start gap-2 min-w-0 flex-1">
                                  ...
                                  <div className="space-y-0.5 min-w-0">
                                    <div className="flex items-center gap-1.5 flex-wrap">
                                      ...
                                      <span className={cn(
                                        "text-[7.5px] font-black uppercase px-1.5 py-0.2 rounded font-mono",
                                        loc.type === 'Godown'
                                      ...
                                      {loc.isDefault && (
                                        <span className="text-[7.5px] font-extrabold uppercase px-1 py-0.2 rounded bg-slate-200 dark:bg-neutral-800 text-slate-600 dark:text-slate-400 font-mono">
                                          Default
                                        </span>
                                      )}
                                    </div>
                                    ...
                                    {loc.phone && (
                                      <p className="text-[8.5px] text-slate-400 dark:text-slate-500 font-mono">
                                        📞 {loc.phone}
                                      </p>
                                    )}
                                  </div>
                                ...
                                <div className="shrink-0 pt-0.5">
                                  {isSelected ? (
                                    ...
                                    <div className="w-5 h-5 rounded-full border border-slate-300 dark:border-neutral-700 bg-white dark:bg-neutral-800" />
                                  )}
                                </div>
                              </div>
                            );
