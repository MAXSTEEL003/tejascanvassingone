...
export default function NotificationSettings() {
  ...
  const [testSupplierEmail, setTestSupplierEmail] = useState('orders@tejascanvassing.com');
  const [testPoNumber, setTestPoNumber] = useState('TC-9801');
  const [testPhone, setTestPhone] = useState('9945351166');
  const [testRecipientName, setTestRecipientName] = useState('Registered Merchant');
  ...
  const [testMessageText, setTestMessageText] = useState(
    'Hello Wholesale Partner,\n\nYour Purchase Order TC-9801 for 24 QTLS (KESHAR KALI) has been successfully dispatched to supplier DIRECT RICE MILL.\n\nArrival ETA: 3-5 business days at APMC Yard, Bangalore.\n\nTejas Canvassing Brokerage Desk'
  );
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.aisensyApiKeyPrefix) {
          setAiSensyKeyPrefix(data.aisensyApiKeyPrefix);
        }
        if (data.aisensyCampaignName) {
          setCustomAiSensyCampaign(data.aisensyCampaignName);
        }
      }
    } catch (err) {
  ...
  const handleSendTestPO = async () => {
    ...
    try {
      const emailHtml = generateSupplierPOEmailHtml(
        ...
        [{
          id: testPoNumber,
          buyer: testRecipientName || 'Registered Merchant',
          product: 'KESHAR KALI',
          qty: 80,
          rate: 8400,
          date: new Date(),
  ...
  const [customAiSensyKey, setCustomAiSensyKey] = useState<string>('');
  const [customAiSensyCampaign, setCustomAiSensyCampaign] = useState<string>('order_notification');
  const [aiSensyKeyPrefix, setAiSensyKeyPrefix] = useState<string | null>(null);
  const [isVerifyingAiSensy, setIsVerifyingAiSensy] = useState<boolean>(false);
  const [aiSensyVerifyFeedback, setAiSensyVerifyFeedback] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const handleVerifyAndSaveAiSensy = async (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    if (!customAiSensyKey.trim()) {
      setAiSensyVerifyFeedback({
        type: 'error',
        text: 'Please provide your active AiSensy API Key.'
      });
      return;
    }
    setIsVerifyingAiSensy(true);
    setAiSensyVerifyFeedback(null);
    try {
      const res = await fetch('/api/update-aisensy-credentials', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          apiKey: customAiSensyKey.trim(),
          campaignName: customAiSensyCampaign.trim() || 'order_notification'
        })
      });
      ...
      if (res.ok && data.success) {
        setAiSensyVerifyFeedback({
          type: 'success',
          text: data.message || 'AiSensy credentials connected successfully!'
        });
        setGatewayStatus(prev => ({ ...prev, aisensy: true }));
        if (data.apiKeyPrefix) setAiSensyKeyPrefix(data.apiKeyPrefix);
        fetchHistory();
      } else {
        setAiSensyVerifyFeedback({
          type: 'error',
          text: data.error || 'Failed to save AiSensy credentials.'
        });
      }
    } catch (err: any) {
      setAiSensyVerifyFeedback({
        type: 'error',
        text: `Network error connecting to AiSensy: ${err.message || 'Unknown error'}`
      });
    } finally {
      setIsVerifyingAiSensy(false);
    }
  };
  ...
  const [customSmtpPort, setCustomSmtpPort] = useState<string>('587');
  const [customSmtpUser, setCustomSmtpUser] = useState<string>('tejasadinarayan@gmail.com');
  const [customSmtpPass, setCustomSmtpPass] = useState<string>('');
  const [customSmtpFrom, setCustomSmtpFrom] = useState<string>('tejasadinarayan@gmail.com');
  const [isVerifyingSmtp, setIsVerifyingSmtp] = useState<boolean>(false);
  const [smtpVerifyFeedback, setSmtpVerifyFeedback] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const handleVerifyAndSaveSmtp = async (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    if (!customSmtpUser.trim() || !customSmtpPass.trim()) return;
    setIsVerifyingSmtp(true);
    setSmtpVerifyFeedback(null);
    try {
      const res = await fetch('/api/update-smtp-credentials', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          host: customSmtpHost.trim(),
          port: parseInt(customSmtpPort.trim(), 10) || 587,
          user: customSmtpUser.trim(),
          pass: customSmtpPass.trim(),
          fromEmail: customSmtpFrom.trim() || customSmtpUser.trim()
        })
      });

      const data = await res.json();
      if (res.ok && data.success) {
        setSmtpVerifyFeedback({
          type: 'success',
          text: data.message || 'SMTP credentials verified and connected successfully!'
        });
        setGatewayStatus(prev => ({ ...prev, smtp: true }));
        fetchHistory();
      } else {
        setSmtpVerifyFeedback({
          type: 'error',
          text: data.error || 'SMTP authentication failed. If using Gmail, make sure to generate an App Password in your Google Account security settings.'
        });
      }
    } catch (err: any) {
      setSmtpVerifyFeedback({
        type: 'error',
        text: `Network error verifying SMTP credentials: ${err.message || 'Unknown error'}`
      });
    } finally {
  ...
  const handleSendTestWhatsApp = async () => {
    ...
    try {
      ...
      if (customAiSensyKey.trim()) {
        payload.aisensyApiKey = customAiSensyKey.trim();
      ...
      if (res.ok && data.success) {
        if (data.gateway === 'aisensy') {
          ...
          setTestFeedback({
            type: 'success',
            text: `WhatsApp dispatch recorded & direct link generated! Save your AiSensy API Key below to transmit live messages automatically.`
          });
        }
      } else {
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        ...
        // Initial fallbacks
        setBusinessName(localStorage.getItem('userName') || 'Registered Merchant');
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (address) stakeholderData.address = address;
      if (stateVal) stakeholderData.state = stateVal;
      if (mandi) stakeholderData.mandi = mandi;
      if (bankName) stakeholderData.bankName = bankName;
      if (accountNo) stakeholderData.accountNo = accountNo;
      if (ifsc) stakeholderData.ifsc = ifsc;
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input 
                      ...
                      placeholder="e.g. Registered Merchant"
                    />
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className={cn(
                ...
                <div className={cn(
                  ...
                  gatewayStatus.aisensy ? "bg-emerald-500/20 text-emerald-600 dark:text-emerald-300" : "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400"
                )}>
                  <MessageSquare className="w-5 h-5" />
                </div>
                <div className="space-y-1">
                  <div className="flex items-center justify-between gap-2">
                    <h4 className="font-black text-xs uppercase tracking-wider">AiSensy WhatsApp API</h4>
                    <span className={cn(
                      "px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest",
                      gatewayStatus.aisensy ? "bg-emerald-600 text-white" : "bg-amber-500/20 text-amber-800 dark:text-amber-200"
                    )}>
                      {gatewayStatus.aisensy ? 'Live AiSensy Active' : 'Setup Required'}
                    </span>
                  </div>
                  <p className="text-[11px] opacity-80 leading-relaxed font-medium">
                    {gatewayStatus.aisensy 
                      ? 'Connected to AiSensy. Automated WhatsApp notifications route directly via AiSensy campaign templates.'
                      : 'AiSensy credentials pending. Enter your AiSensy API Key below to activate automated messaging.'}
                  </p>
                </div>
              </div>
                ...
                >
                  {isSendingTestWa ? (
                    <>
                      ...
                      {gatewayStatus.aisensy ? 'Sending via AiSensy...' : 'Dispatching via Gateway...'}
                    </>
                  ) : (
                    <>
                      <Send className="w-4 h-4 text-emerald-600" />
                      {gatewayStatus.aisensy ? 'Send via AiSensy Automated API' : 'Send via Automated WhatsApp API'}
                    </>
                  )}
                </button>
              </div>

              {/* AiSensy WhatsApp API Configuration Card (Primary) */}
              <div className="p-4 sm:p-5 rounded-2xl bg-surface-container border border-emerald-500/30 space-y-4 shadow-sm">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h4 className="text-xs font-black uppercase tracking-wider text-on-surface flex items-center gap-2">
                        AiSensy WhatsApp API Credentials & Campaign
                        <span className={cn(
                          "px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest",
                          gatewayStatus.aisensy ? "bg-emerald-600 text-white" : "bg-amber-500/20 text-amber-800 dark:text-amber-300"
                        )}>
                          {gatewayStatus.aisensy ? 'Live & Connected' : 'Configuration Required'}
                        </span>
                      </h4>
                      <p className="text-[10px] text-secondary font-medium">
                        Primary automated WhatsApp messaging service for order dispatches, trade notes, and buyer alerts.
                      </p>
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={() => setShowAiSensyConfig(!showAiSensyConfig)}
                    className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 hover:underline cursor-pointer"
                  >
                    {showAiSensyConfig ? 'Hide Credentials' : 'Update Credentials'}
                  </button>
                </div>

                {showAiSensyConfig && (
                  <form onSubmit={handleVerifyAndSaveAiSensy} className="space-y-3 pt-2 border-t border-outline-variant/30">
                    <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3 text-[11px] text-emerald-950 dark:text-emerald-200 space-y-1">
                      <p className="font-bold">How to connect your AiSensy WhatsApp API:</p>
                      <ol className="list-decimal list-inside space-y-0.5 text-[10px] opacity-90">
                        <li>Log in to your <strong>AiSensy Dashboard</strong> (<a href="https://app.aisensy.com" target="_blank" rel="noopener noreferrer" className="underline font-bold">app.aisensy.com</a>).</li>
                        <li>Navigate to <strong>Manage ➔ API Key</strong> and copy your live key.</li>
                        <li>Paste your API key below and verify your <strong>Campaign Name</strong> (default: <code className="font-mono font-bold">order_notification</code>).</li>
                      </ol>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="text-[9px] font-black uppercase tracking-widest text-secondary block">
                          AiSensy API Key {aiSensyKeyPrefix && <span className="text-emerald-600 font-mono">({aiSensyKeyPrefix})</span>}
                        </label>
                        <input
                          type="password"
                          value={customAiSensyKey}
                          onChange={(e) => setCustomAiSensyKey(e.target.value)}
                          placeholder={aiSensyKeyPrefix ? `Current: ${aiSensyKeyPrefix} (enter new key to replace)` : "Paste AiSensy API Key..."}
                          className="w-full bg-surface-container-low border border-outline-variant rounded-lg px-3 py-2 text-xs font-mono font-bold outline-none text-on-surface focus:ring-2 focus:ring-emerald-500/20"
                        />
                      </div>

                      <div className="space-y-1">
                        <label className="text-[9px] font-black uppercase tracking-widest text-secondary block">
                          Approved Campaign Name / Template
                        </label>
                        <input
                          type="text"
                          value={customAiSensyCampaign}
                          onChange={(e) => setCustomAiSensyCampaign(e.target.value)}
                      ...
                      {aiSensyVerifyFeedback && (
                        <motion.div
                          ...
                          className={cn(
                            ...
                            aiSensyVerifyFeedback.type === 'success'
                              ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30"
                              : "bg-rose-500/10 text-rose-700 dark:text-rose-300 border border-rose-500/30"
                          )}
                        >
                          {aiSensyVerifyFeedback.type === 'success' ? <CheckCircle2 className="w-4 h-4 shrink-0" /> : <AlertCircle className="w-4 h-4 shrink-0" />}
                          <span className="flex-1">{aiSensyVerifyFeedback.text}</span>
                        </motion.div>
                      )}
                    </AnimatePresence>
                    ...
                    <div className="flex justify-end pt-1">
                      <button
                        type="submit"
                        disabled={isVerifyingAiSensy || !customAiSensyKey.trim()}
                        className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-black uppercase tracking-wider transition-all disabled:opacity-50 flex items-center gap-2 cursor-pointer shadow-md shadow-emerald-600/20"
                      >
                        {isVerifyingAiSensy ? (
                          <>
                            <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                            Connecting to AiSensy...
                          </>
                        ) : (
                          <>
                            <CheckCircle2 className="w-4 h-4" />
                            Connect & Save AiSensy Credentials
                          </>
                        )}
                      </button>
                    </div>
              ...
              <div className="flex items-center justify-between pb-4 border-b border-outline-variant/30">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-secondary font-medium">Verify compilation and transmission of supplier POs and buyer order confirmations.</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1 bg-primary/10 hover:bg-primary/20 text-primary rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer"
                  >
                    {showSmtpConfig ? 'Hide SMTP Form' : 'Configure SMTP'}
                  </button>
                  <span className={cn(
                    "px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider",
                    gatewayStatus.smtp ? "bg-emerald-600 text-white" : "bg-amber-500/20 text-amber-800 dark:text-amber-200"
                  )}>
                    {gatewayStatus.smtp ? 'Live Relay Active' : 'Simulation Mode'}
                  </span>
                </div>
              </div>

              {/* SMTP Credential Form */}
              {showSmtpConfig && (
                <form onSubmit={handleVerifyAndSaveSmtp} className="p-4 bg-surface-container rounded-2xl border border-outline-variant/50 space-y-3">
                  <div className="flex items-center gap-2">
                    <div className="p-1.5 bg-primary/10 text-primary rounded-lg">
                      <Key className="w-3.5 h-3.5" />
                    </div>
                    <span className="text-xs font-black uppercase tracking-wider text-on-surface">
                      SMTP Outgoing Email Credentials
                    </span>
                  </div>

                  <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 text-[11px] text-on-surface space-y-1">
                    <p className="font-bold">Gmail Setup Notice:</p>
                    <p className="text-[10px] text-secondary">
                      For Gmail (e.g. <code className="font-mono font-bold text-on-surface">tejasadinarayan@gmail.com</code>), use your standard Gmail address and generate an <strong>App Password</strong> under <strong>Google Account &gt; Security &gt; 2-Step Verification &gt; App passwords</strong>.
                    </p>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="space-y-1">
                      <label className="text-[9px] font-black uppercase tracking-widest text-secondary block">SMTP Host</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-1">
                      <label className="text-[9px] font-black uppercase tracking-widest text-secondary block">SMTP Port</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-1">
                      <label className="text-[9px] font-black uppercase tracking-widest text-secondary block">Sender / User Email</label>
                      <input
                        type="email"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="text-[9px] font-black uppercase tracking-widest text-secondary block">App Password</label>
                      <input
                        type="password"
                    ...
                    <div className="flex items-end">
                      <button
                        type="submit"
                        disabled={isVerifyingSmtp || !customSmtpUser || !customSmtpPass}
                        className="w-full px-4 py-2 bg-primary hover:bg-primary/90 text-white rounded-lg text-xs font-black uppercase tracking-wider transition-all disabled:opacity-50 flex items-center justify-center gap-1.5 cursor-pointer shadow-sm"
                      >
                        {isVerifyingSmtp ? (
                          <>
                            <div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  ...
                  {smtpVerifyFeedback && (
                    <div className={cn(
                      ...
                      smtpVerifyFeedback.type === 'success' ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30" : "bg-rose-500/10 text-rose-700 dark:text-rose-300 border border-rose-500/30"
                    )}>
                      {smtpVerifyFeedback.type === 'success' ? <Check className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}
                      <span>{smtpVerifyFeedback.text}</span>
                    </div>
                  )}
                </form>
              )}
